...
export const updatedTour = createAsyncThunk("tour/updateTour", async ({ id, updatedTourData, toast, navigate }, { rejectWithValue }) => {

    try {
        const response = await api.updatedTour(updatedTourData, id);
...
const tourSlice = createSlice({
    ...
    extraReducers: {
        ...
        [updatedTour.pending]: (state, action) => {
            state.loading = true;
        },
        [updatedTour.fulfilled]: (state, action) => {
            state.loading = false;
            const { arg: { id } } = action.meta
            if (id) {
        ...
        [updatedTour.rejected]: (state, action) => {
            state.loading = false;
            state.error = action.payload.message;
        },
